...
\begin{macro}{changelog}\oarg{options}\AfterLastParam
Wraps \env{description} while providing the \env{version} environment and
the \cs{shortversion} command. In addition to the options below,
\meta{options} may contain any of the options for \env{version} (see
...
\begin{Optionlist}
	section & Insert a \cs{section} before the changelog? Default: true \\
	sectioncmd & Which sectioning command to use? Default: \cs{section} \\
...
\shortversion{v=1.0.0, date=2018-10-26, changes=First stable release}
